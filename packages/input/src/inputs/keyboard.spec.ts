...
describe('The `Keyboard` class', () => {
  ...
  describe('should have a `key()` method that returns a control that', () => {
    ...
    describe('when queried', () => {
      ...
      it('returns `true` when any key is pressed', () => {
        ...
        expect(keyboard.key('any').query()).to.equal(true);

        doc.keyUp('a');
      });

      it('returns `false` when any key is not pressed', () => {
        expect(keyboard.key('any').query()).to.equal(false);
      });
    });
  ...
  describe('should have a `directionalKeys()` method that returns a control that', () => {
    it('returns the correct label', () => {
      ...
      expect(keyboard.directionalKeys(['z', 'g', 'h', 'j']).label).to.equal(
        'ZGHJ',
      );
    });

    it("throws an error when the passed in arrow key template doesn't exist", () => {
      expect(() => keyboard.directionalKeys('wsad').query()).to.throw(
        Error,
        'Directional key template "wsad" not found!',
      );
    });
    ...
    it('accepts custom sets of arrow keys', () => {
      ...
      expect(
        keyboard.directionalKeys(['T', 'H', 'U', 'G']).query(),
      ).to.deep.equal(new Vector2(-1, 0));
      doc.keyUp('H');
    });

    it('throws an error when the given number of keys is not four', () => {
      expect(() => keyboard.directionalKeys(['c'])).to.throw(
        Error,
